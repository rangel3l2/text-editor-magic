...
DROP POLICY IF EXISTS "Admins and moderators can delete roles" ON public.user_roles;
...
DROP POLICY IF EXISTS "Admins and moderators can read all profiles" ON public.profiles;
...
-- Drop and recreate policies for academic_work_types
DROP POLICY IF EXISTS "Admins can manage work types using role function" ON public.academic_work_types;
DROP POLICY IF EXISTS "Admins and moderators can manage work types" ON public.academic_work_types;

CREATE POLICY "Staff can manage work types"
...
DROP POLICY IF EXISTS "Admins and moderators can view all logs" ON public.system_logs;
